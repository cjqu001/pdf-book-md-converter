...
README_MD = r"""# PDF Book to Markdown Converter
...
CONVERT_BOOKS_PY = r"""#!/usr/bin/env python3
# -*- coding: utf-8 -*-
...
def ensure_imports() -> None:
    global fitz, pdfplumber, Image, pytesseract
    if fitz is None:
...
def main():
    repo = Path("pdf-book-md-converter")
    repo.mkdir(exist_ok=True)

    (repo / "README.md").write_text(README_MD, encoding="utf-8")
    (repo / "convert_books.py").write_text(CONVERT_BOOKS_PY, encoding="utf-8")
    (repo / "requirements.txt").write_text(REQUIREMENTS_TXT, encoding="utf-8")
    (repo / ".gitignore").write_text(GITIGNORE, encoding="utf-8")

    print(f"Created repo at: {repo.resolve()}")
    print("Files created:")
    for name in ["README.md", "convert_books.py", "requirements.txt", ".gitignore"]:
        print(f" - {repo / name}")
